...
if __name__ == "__main__":
    ...
    parser.add_argument('--root_dir', type=str, required=True, help='Root directory for input and output data.')
    parser.add_argument('--out_dir_263dim', type=str, required=True, help='Output directory for 263D results.')
    parser.add_argument('--out_dir_smpl', type=str, required=True, help='Output directory for SMPL results.')
    ...
    # sample_xxxx.npy
    # seq_names = sorted(seq_names, key=lambda x: int(x.split('_')[-1].split('.')[0]))
    ...
    # total_splits = 4
    # seq_names = seq_names[args.split_idx::total_splits] if args.split_idx >= 0 else seq_names


    # TODO: temporary speed up by reversing the list to avoid two processes working on same files
    # seq_names = list(reversed(seq_names))

    # print(seq_names[:2000])
    # breakpoint()

    # # Initialize pipeline
    # pipeline = SMPLXTo263Pipeline()

    # for seq_name in tqdm(seq_names, desc="Processing sequences"):

    #     input_file = os.path.join(args.root_dir, seq_name)
    #     output_file = os.path.join(args.out_dir, seq_name)
    ...
    #         'body_pose': pred_data['pose'],
    #         'transl': pred_data['transl'],
    #         'betas': np.zeros((pred_data['orient'].shape[0], 10)),  # Assuming zero betas
    #     }
    ...
    #     # Save results
    #     np.save(output_file, motion_263)
